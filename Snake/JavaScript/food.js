...
function createNewFoodSegment() {
  const newFoodSegment = [
    {x: Math.floor(Math.random() * 21) + 1, y: Math.floor(Math.random() * 21) + 1}
  ]
  if (isOnSnake(newFoodSegment[0])) createNewFoodSegment()
  else foodSegments.push(newFoodSegment[0])
}
...
export function initFood() {
  ...
  for (let i = 0; i < amountOfFoodSegments; i++) createNewFoodSegment()
